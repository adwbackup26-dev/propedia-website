          ...
          <div className="detail-hero" style={{ position:'relative', height:400, background:BG[photoIdx%BG.length], overflow:'hidden' }}>
            ...
            <div style={{ position:'absolute', bottom:0, left:0, right:0, background:'rgba(0,0,0,.75)', padding:'12px 18px 38px' }}>
              <div style={{ fontSize:26, fontWeight:600, letterSpacing:'-.03em', marginBottom:4 }}>
                ...
                {listing.OriginalListPrice&&listing.OriginalListPrice>listing.ListPrice&&<span style={{ fontSize:13, fontWeight:400, color:'rgba(255,255,255,.32)', textDecoration:'line-through', marginLeft:8 }}>{fmt(listing.OriginalListPrice)}</span>}
              </div>
              <div style={{ display:'flex', gap:14, fontSize:11, color:'rgba(255,255,255,.5)', flexWrap:'wrap' }}>
                <span><i className="ti ti-map-pin"/> {address}</span>
                {listing.BedroomsTotal!=null&&<span><i className="ti ti-bed"/> {listing.BedroomsTotal} bd</span>}
                {listing.BathroomsTotalInteger!=null&&<span><i className="ti ti-bath"/> {listing.BathroomsTotalInteger} ba</span>}
                {listing.DaysOnMarket!=null&&<span><i className="ti ti-clock"/> {listing.DaysOnMarket} days</span>}
